...
const ContentDetailPayment = () => {
  ...
  const chosen_hotel = useSelector(
    state => state?.reservation?.data?.chosen_hotel?.data?.get_chosen_hotel,
  );
  ...
  const dateCheckIn = new Date(chosen_hotel?.chosen_hotel_params?.check_in);
  const dateCheckOut = new Date(chosen_hotel?.chosen_hotel_params?.check_out);

  const reservationNight =
    (dateCheckOut.getTime() - dateCheckIn.getTime()) / (1000 * 3600 * 24);

  return chosen_hotel !== undefined ? (
    <ScrollView>
      <SafeAreaView>
          ...
          <Surface mode="flat" style={[styles.card, styles.cardHotel]}>
            <View style={styles.cardBody}>
              <View style={styles.cardImage} resizeMode="cover">
                <Image
                  source={{
                    uri: chosen_hotel?.chosen_hotel_detail?.images[5]
                      ?.thumbnail,
                  }}
                  style={styles.image}
                />
              ...
              <View style={styles.cardContent}>
                <Text variant="bodyLarge" style={styles.cardTitle}>
                  {chosen_hotel?.chosen_hotel_detail?.hotel_name}
                </Text>
                <Text variant="bodySmall" style={styles.cardSubTitle}>
                  {chosen_hotel?.chosen_hotel_room?.room_name}
                </Text>
                <Text variant="bodySmall" style={styles.cardSubTitle}>
                  {chosen_hotel?.chosen_hotel_params?.total_room} Kamar{' '}
                  {'\u25CF'} Double {'\u25CF'} {reservationNight} Malam
                </Text>
              </View>
            </View>
          </Surface>
          ...
          <View style={styles.status}>
            ...
            <View style={styles.statusNote}>
              <FontAwesome6
                solid
                name={
                  chosen_hotel?.chosen_hotel_prices?.is_refundable
                    ? 'money-bill-transfer'
                    : 'sack-xmark'
                }
                size={19}
              ...
              <Text
                variant="titleSmall"
                style={{
                  color: chosen_hotel?.chosen_hotel_prices?.is_refundable
                    ? '#FFBA7B'
                    : '#FF7B7B',
                  fontWeight: 'bold',
                }}>
